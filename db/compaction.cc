...
Compaction::Compaction(CompactionParams&& params)
    : input_vstorage_(params.input_version),
      ...
      max_subcompactions_(1),  // set 1, TODO remove it
  ...
  if (input_range_.empty()) {
    input_range_.push_back(infinite_range);
  }
  if (!lazy_compaction_) {
...
std::vector<SelectedRange> Compaction::install_range() {
  ...
  if (is_fake_installed_) {
    ...
    for (auto& sub_compact_state : compaction_state_->sub_compact_states) {
      for (auto& sr : sub_compact_state.compaction->input_range_) {
        install_ranges.push_back(sr);
      }
    }
  } else {
    return input_range();
